...
RobotParams parseRobotParams(const std::string &filePath)
{
    ...
    if (config["robot"])
    {
        ...
        if (robotNode["serial_devices"])
        {
            for (const auto &deviceNode : robotNode["serial_devices"])
            {
                SerialDeviceParams device;
                readConfigParam(deviceNode, "serial_id", device.serial_id);
                readConfigParam(deviceNode, "path", device.path);
                readConfigParam(deviceNode, "vid", device.vid, -1);
                readConfigParam(deviceNode, "pid", device.pid, -1);
                readConfigParam(deviceNode, "name", device.name, std::string());
                params.serial_devices.push_back(device);
            }
        }
