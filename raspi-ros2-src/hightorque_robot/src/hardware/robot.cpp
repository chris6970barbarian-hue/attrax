...
namespace hightorque_robot
{
    ...
    static bool get_serial_vid_pid(const std::string& path, int& vid, int& pid)
    {
        struct sp_port *port = nullptr;
        if (sp_get_port_by_name(path.c_str(), &port) != SP_OK || port == nullptr)
        {
            return false;
        }

        const bool ok = sp_get_port_usb_vid_pid(port, &vid, &pid) == SP_OK;
        sp_free_port(port);
        return ok;
    }
    ...
    void robot::init_ser()
    {
        ...
        bool used_serial_device_mapping = false;
        if (!robot_params.serial_devices.empty())
        {
            for (const auto& device : robot_params.serial_devices)
            {
                if (device.serial_id <= 0)
                {
                    std::ostringstream error;
                    error << "Invalid serial_devices serial_id " << device.serial_id;
                    throw std::runtime_error(error.str());
                }

                if (!path_exists(device.path))
                {
                    std::cerr << "Warning: Configured serial device path does not exist: "
                              << device.path << ". Falling back to " << Serial_Type << " scan." << std::endl;
                    continue;
                }

                int detected_vid = -1;
                int detected_pid = -1;
                const bool has_expected_usb_id = device.vid >= 0 || device.pid >= 0;
                const bool has_detected_usb_id = get_serial_vid_pid(device.path, detected_vid, detected_pid);
                if (has_expected_usb_id && !has_detected_usb_id)
                {
                    std::cerr << "Warning: Could not read VID/PID for configured serial device: "
                              << device.path << ". Continuing because the explicit path exists." << std::endl;
                }

                if (has_detected_usb_id &&
                    ((device.vid >= 0 && detected_vid != device.vid) ||
                     (device.pid >= 0 && detected_pid != device.pid)))
                {
                    std::ostringstream error;
                    error << "Configured serial device " << device.path << " VID/PID mismatch. Expected "
                          << std::hex << "0x" << device.vid << ":0x" << device.pid
                          << ", detected 0x" << detected_vid << ":0x" << detected_pid << std::dec;
                    throw std::runtime_error(error.str());
                }

                if (static_cast<int>(str.size()) < device.serial_id)
                {
                    str.resize(device.serial_id);
                }
                str[device.serial_id - 1] = device.path;
                used_serial_device_mapping = true;
                std::cout << "Serial Port" << (device.serial_id - 1) << " = " << device.path;
        ...
        if (ports.empty() && !used_serial_device_mapping)
        {
            std::ostringstream error;
            error << "No serial devices found matching prefix " << Serial_Type;
            throw std::runtime_error(error.str());
        }
        ...
        for(auto board_params: robot_params.CANboards)
        {
            ...
            for(auto port_params: board_params.second.CANports)
            {
                ...
                {
                    std::ostringstream error;
                    error << "Configured serial_id " << serial_id
                          << " for CAN board port exceeds configured/detected serial ports ("
                          << str.size() << ").";
                    throw std::runtime_error(error.str());
                }

                if (str[serial_id - 1].empty())
                {
                    std::ostringstream error;
                    error << "Configured serial_id " << serial_id << " has no mapped serial device path.";
                    throw std::runtime_error(error.str());
                }
